...
C++20 으로 만든 멀티스레드 채팅 서버. 채팅이라는 가벼운 도메인 위에 **데이터 정합성과 동시성을 지키는 구조** 를 얹는 데 초점을 맞췄다. 트랜잭션 단위 flush, 낙관적 동시성 제어(OCC), 커넥션 풀, prepared statement, 그리고 요청이 몰려도 워커가 멈추지 않는 I/O 구조다.
...
(바이너리 패킷)   (asio 코루틴 · JobQueue)   (트랜잭션 · OCC · 풀)   (User · Friendship)
```

| 영역 | 목표 | 이 프로젝트에서 구현한 것 |
|---|---|---|
| 클라이언트 · 프로토콜 | 패킷 정의와 코드가 어긋나지 않게 | Windows C++ 콘솔 클라이언트, protobuf 바이너리 프로토콜, 패킷 핸들러 자동 생성 |
| 데이터 정합성 | 동시에 갱신해도 값이 틀어지지 않게 | `DBContext` 트랜잭션 단위 flush, OCC 충돌 감지, 커넥션 풀 RAII, prepared statement |
| 대량 요청 처리 | 요청이 몰려도 지연 없이 | asio 코루틴, 자원별 mutex / shared_mutex 구분, TLS 송신 버퍼, Room 단위 JobQueue 직렬화 |
| 보안 · 장애 대응 | 인젝션 차단, 장애 격리와 복구 | prepared statement, 화이트리스트 입력 검증, bcrypt, 실패 시 전체 Rollback, 풀 반납 보장 |
| AI 서비스 | 외부 API 를 붙여도 워커가 멈추지 않게 | Claude API 를 같은 io_context 위에서 비동기 스트리밍, 이력 DB 저장, 유저·일 단위 한도, 실패 분류·재시도 |
...
잔고가 걸린 가장 기본적인 트랜잭션 시나리오. `Handle_C_TRANSFER` (`Server/Network/ClientPacketHandler.cpp`).
...
![AI 모드 채팅 — 실제 Claude API 응답. 두 번째 질문 "그럼 세마포어랑 뭐가 달라?" 는 앞 대화가 이력으로 함께 실려 문맥이 이어진다](docs/images/ai_chat.png)
...
- **한도는 DB 에 누적.** `AiUsage(UserId, Day)` 에 호출 수·입력/출력 토큰을 누적. 일일 호출·토큰 상한 초과 시 거절.
...
- **실제 엔드포인트**: 더미 키로는 `HTTP 401: API key is invalid.` (TLS·인증서·SNI·파싱이 실제 서버에서 동작), 유효한 키로는 위 캡처처럼 스트리밍 응답을 받았다. 측정한 호출 한 건 기준 첫 조각까지 약 1.5초, 완료까지 약 3초 (입력 168 / 출력 76 토큰, effort=low).
...
### 왜 직접 만들었나 — 정합성이 중요한 곳은 raw SQL 을 쓰는데

정합성과 성능이 중요한 시스템은 보통 ORM 없이 SQL 을 직접 짠다. 실행 계획을 통제해야 하기 때문이다. 그걸 알면서 ORM 을 만든 이유는 하나다. **SQL 빌더 계층(L5) 을 직접 짰기 때문에 어떤 SQL 이 나가는지 전부 안다.** UPDATE 는 dirty 컬럼만, WHERE 는 origin 전체, 값은 전부 바인딩. 필요하면 `DBConnection::Execute` 로 raw SQL 을 그대로 쓴다. ORM 은 SQL 을 숨기는 도구가 아니라 SQL 을 이해한 결과물이어야 한다는 생각으로 설계했다.
